...
public class AccountBalanceHistoryRepository(DatabaseContext databaseContext) : IAccountBalanceHistoryRepository
{
    ...
    /// <inheritdoc/>
    public AccountBalanceHistory FindEarliestByTransactionId(AccountId accountId, TransactionId transactionId) =>
        databaseContext.AccountBalanceHistories
    ...
    /// <inheritdoc/>
    public IReadOnlyCollection<(AccountBalanceHistory History, Transaction Transaction)> FindAllHistoriesLaterThanOrEqualTo(AccountId accountId, DateOnly historyDate, int sequence)
    {

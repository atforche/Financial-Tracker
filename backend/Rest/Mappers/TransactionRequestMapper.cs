...
/// <summary>
/// Mapper class that handles mapping transaction request models to domain transaction requests.
/// </summary>
...
{
    /// <summary>
    /// Attempts to map the provided create model to a domain create request.
    /// </summary>
        ...
        out Dictionary<string, string[]> errors)
    {
        ...
        TransactionTypeModel requestType = TransactionTypeMapper.ToModel(model);
        if (TransactionTypeMapper.ToModel(transaction.Type) != requestType)
        {
    ...
    {
        if (accountMapper.TryToDomain(accountId, out account))
        {
            return true;
        }
    ...
    {
        if (fundMapper.TryToDomain(fundId, out fund))
        {
            return true;
        }

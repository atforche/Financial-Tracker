...
/// <summary>
/// Class that handles retrieving Transaction dashboard data for a date range or Accounting Period range.
/// </summary>
public class TransactionDashboardGetter(
...
{
    /// <summary>
    /// Retrieves the Transaction dashboard data that matches the specified criteria.
    /// </summary>
    public bool TryGet(
        TransactionDashboardQueryParameterModel request,
        ...
        out Dictionary<string, string[]> errors)
    {
        ...
        if (hasDateRangeParameters == hasAccountingPeriodRangeParameters)
        {
            ...
            results = CreateEmptyResult(TransactionDashboardModeModel.AccountingPeriod);
            return false;
        }
        ...
        out TransactionDashboardModel results)
    {
        ...
        return BuildResult(
            ...
            requestedFundNames,
            TransactionDashboardModeModel.AccountingPeriod,
        ...
        out TransactionDashboardModel results)
    {
        ...
        {
            results = CreateEmptyResult(TransactionDashboardModeModel.Date);
            return false;
        }
        ...
        return BuildResult(
            ...
            requestedAccountNames,
            requestedFundNames,
            TransactionDashboardModeModel.Date,
    ...
    private bool BuildResult(
        ...
        IReadOnlySet<string>? requestedAccountNames,
        IReadOnlySet<string>? requestedFundNames,
        TransactionDashboardModeModel mode,
        DateOnly startDate,
        DateOnly endDate,
        ...
        out TransactionDashboardModel results)
    {
        ...
        results = new TransactionDashboardModel
        {
            Mode = mode,
    ...
    private static IEnumerable<TransactionModel> ApplyPaging(
        ...
        TransactionDashboardQueryParameterModel request) => transactions
        .Skip(request.Offset ?? 0)
        .Take(request.Limit ?? int.MaxValue);

    private static TransactionDashboardModel CreateEmptyResult(TransactionDashboardModeModel mode) => new()
    {
        Mode = mode,
        Transactions = new CollectionModel<TransactionModel>
        {
    ...
    private static List<TransactionModel> SortTransactions(
        ...
        TransactionSortOrderModel? sort) => sort switch
        {
            ...
            TransactionSortOrderModel.DebitFromDescending => transactions.OrderByDescending(GetDebitFrom).ThenByDescending(transaction => transaction.Date).ThenByDescending(transaction => transaction.Sequence).ToList(),
            TransactionSortOrderModel.CreditTo => transactions.OrderBy(GetCreditTo).ThenByDescending(transaction => transaction.Date).ThenByDescending(transaction => transaction.Sequence).ToList(),
            TransactionSortOrderModel.CreditToDescending => transactions.OrderByDescending(GetCreditTo).ThenByDescending(transaction => transaction.Date).ThenByDescending(transaction => transaction.Sequence).ToList(),
    ...
    private static List<TransactionDashboardTransactionTypeSummaryModel> BuildTransactionTypeSummaries(
        IReadOnlyCollection<TransactionModel> transactions) => transactions
        .GroupBy(transaction => transaction.TransactionType)
        .OrderBy(group => group.Key)
        .Select(group => new TransactionDashboardTransactionTypeSummaryModel
        {
    ...
    private static List<TransactionDashboardDateSummaryModel> BuildDateSummaries(
        DateOnly startDate,
        DateOnly endDate,
        IReadOnlyCollection<TransactionModel> transactions)
    {
        ...
        return new DateRange(startDate, endDate)
            ...
            .Select(date =>
            {
                ...
                dateTransactions ??= [];
                return new TransactionDashboardDateSummaryModel
                {
                    Date = date,

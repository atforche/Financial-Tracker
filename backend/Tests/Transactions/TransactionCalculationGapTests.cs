...
public sealed class TransactionCalculationGapTests
{
    ...
    {
        CreateTransactionResultModel created = await test.Api.PostAsync<CreateTransactionModel, CreateTransactionResultModel>("/transactions", new CreateIncomeTransactionModel { AccountingPeriodId = period.Id, Date = new DateOnly(2026, 7, 15), Description = "Income", Amount = 100m, Source = new CreateIncomeTransactionSourceModel { Location = "Employer", Income = IncomeBreakdownModelFactory.Simple(100m) }, Destinations = [new CreateIncomeTransactionDestinationModel { AccountId = first.Id, Amount = 40m, FundAssignments = [new CreateIncomeFundAmountModel { FundId = firstFund.Id, Amount = 40m }] }, new CreateIncomeTransactionDestinationModel { AccountId = second.Id, Amount = 60m, FundAssignments = [new CreateIncomeFundAmountModel { FundId = secondFund.Id, Amount = 60m }] }] });
        return new TransactionHandle(created.Id);
    }

    private static UpdateIncomeTransactionModel CreateIncomeUpdate(AccountHandle account, FundHandle fund, decimal amount, DateOnly date) => new() { Date = date, Description = "Income", Amount = amount, Source = new UpdateIncomeTransactionSourceModel { Location = "Employer", Income = IncomeBreakdownModelFactory.Simple(amount) }, Destinations = [new UpdateIncomeTransactionDestinationModel { AccountId = account.Id, Amount = amount, FundAssignments = [new CreateIncomeFundAmountModel { FundId = fund.Id, Amount = amount }] }] };

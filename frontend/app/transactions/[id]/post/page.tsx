...
import PostTransactionForm from "@/app/transactions/[id]/post/PostTransactionForm";
import type { PostTransactionFormSearchParams } from "@/app/transactions/[id]/post/postTransactionFormSearchParams";
...
interface PageProps {
  ...
  readonly searchParams: Promise<PostTransactionFormSearchParams>;
}
...
const Page = async function ({
  ...
  const { account, accountId, accountingPeriodId, fundId } = await searchParams;
  ...
  if (
    ...
    throw new Error(
      `Failed to load data for post transaction page: ${transactionError?.detail ?? accountingPeriodError?.detail ?? accountError?.detail ?? fundError?.detail ?? "Unknown error"}`,
    );
  }

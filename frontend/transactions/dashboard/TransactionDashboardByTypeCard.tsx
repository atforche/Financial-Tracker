...
import type { TransactionDashboard } from "@/transactions/types";
import formatCurrency from "@/framework/formatCurrency";

interface TransactionDashboardByTypeCardProps {
  readonly dashboard: TransactionDashboard;
}

/**
 * Component that displays transaction count and amount broken down by type.
 */
const TransactionDashboardByTypeCard = function ({
  dashboard,
}: TransactionDashboardByTypeCardProps): JSX.Element {
  const maxTransactionCount = Math.max(
    ...dashboard.transactionTypes.map((summary) => summary.totalCount),
    1,
  );
  const maxTransactionAmount = Math.max(
    ...dashboard.transactionTypes.map((summary) => summary.totalAmount),
    1,
  );

  const renderTypeSummary = function (
    summary: TransactionDashboard["transactionTypes"][number],
  ): JSX.Element {
  ...
  if (dashboard.transactionTypes.length === 0) {
    return (
  ...
  return (
    <SummaryCard title="Transactions by type">
      <Stack spacing={2.5}>
        {dashboard.transactionTypes.map(renderTypeSummary)}
      </Stack>
    </SummaryCard>
  );
};

export default TransactionDashboardByTypeCard;

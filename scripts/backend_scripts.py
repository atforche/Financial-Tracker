    ...
    def __init__(self):
        ...
        self.steps.append(
            Step(
                ...
                "Restore completed",
                lambda: self.run_subprocess("dotnet restore ../backend/Backend.sln"),
            )
        )
    ...
    def __init__(self) -> None:
        ...
        self.steps.append(
            Step(
                ...
                "Format completed",
                lambda: self.run_subprocess(
                    "dotnet format ../backend/Backend.sln --verify-no-changes --no-restore --severity info"
                ),
            )
        )
...
class TestBackendSolution(Command):
    ...
    use_database: Annotated[
        bool, "If provided, runs the unit test against an actual database"
    ]
    ...
    def __init__(self):
        ...
        self.steps.append(
            Step("Test Backend Solution", "Tests completed", self.run_tests)
        )
...
class GetBackendCoverage(Command):
    ...
    use_database: Annotated[
        bool, "If provided, runs the tests against an actual database"
    ]
    ...
    def __init__(self):
        ...
        super().__init__(
            "coverage", "Runs backend tests and reports code coverage metrics"
        )
        self.steps.append(
            Step("Get Backend Coverage", "Coverage metrics reported", self.get_coverage)
        )
    ...
    def get_coverage(self):
        ...
        for assembly in sorted(
            ...
            key=lambda package: package.attrib["name"],
        ):
            print(
                f"  {assembly.attrib['name']}: "
...
class RunBackendApi(Command):
    ...
    def run_api(self):
        ...
        print(
            f"Running the backend on port {self.port} with database at {self.database_path}"
        )
...
class CreateMigration(Command):
    ...
    def __init__(self):
        ...
        self.steps.append(
            Step("Create Migration", "New migration created", self.create_new_migration)
        )
    ...
    def create_new_migration(self):
        ...
        self.run_subprocess(
            f"dotnet ef migrations add {self.name} \

...
class SmokeTestContainerImages(Command):
    ...
    def __init__(self):
        ...
        super().__init__("smoke-test", "Starts the built container images and verifies they become healthy")
    ...
    def smoke_test_images(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="financial-tracker-smoke-") as temporary_directory:
            ...
            try:
                ...
                self.run_docker([
                    ...
                    "--publish", "127.0.0.1::3000", "--read-only", "--tmpfs", "/tmp",
                    "--cap-drop", "ALL", "--security-opt", "no-new-privileges:true",
                    "--env", f"API_URL=http://{backend}:8080",
                    "--env", "PUBLIC_ORIGIN=https://localhost",
                    "--env", "GOOGLE_CLIENT_ID=container-smoke-test",
                    "--env", "GOOGLE_CLIENT_SECRET=container-smoke-test",
                    "--env", "GOOGLE_ALLOWED_SUBJECTS=container-smoke-test",

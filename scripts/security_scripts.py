...
class ScanDependencies(Command):
    ...
    def __init__(self):
        ...
        self.steps.append(
            Step(
                ...
                self.scan_frontend_dependencies,
            )
        )
        self.steps.append(
            Step(
                ...
                self.scan_backend_dependencies,
            )
        )
    ...
    def scan_backend_dependencies(self) -> None:
        ...
        if vulnerabilities:
            raise RuntimeError(
                "NuGet reported vulnerable packages: "
    ...
    def get_vulnerabilities(value: Any) -> set[str]:
        ...
        if isinstance(value, list):
            return set().union(
                *(ScanDependencies.get_vulnerabilities(item) for item in value)
            )
        ...
        return set().union(
            *(ScanDependencies.get_vulnerabilities(item) for item in value.values())
        )
...
class ScanContainerImages(Command):
    ...
    def __init__(self):
        ...
        super().__init__(
            "scan-images",
            "Scans deployable container images for high and critical vulnerabilities",
        )
        self.steps.append(
            Step("Scan Backend Image", "Backend image scanned", self.scan_backend_image)
        )
        self.steps.append(
            Step(
                "Scan Frontend Image",
                "Frontend image scanned",
                self.scan_frontend_image,
            )
        )
        self.steps.append(
            Step(
                "Scan Migrator Image",
                "Migrator image scanned",
                self.scan_migrator_image,
            )
        )
        self.steps.append(
            Step("Scan Proxy Image", "Proxy image scanned", self.scan_proxy_image)
        )

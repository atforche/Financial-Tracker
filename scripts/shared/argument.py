...
class Argument(Generic[T]):
    ...
    def __init__(
        ...
        Args:
            ...
            value_type (type[T] | None): Explicit argument type for dynamically discovered arguments
        """
    ...
    def try_parse(self, arg_list: list[str]) -> T | Exception:
        ...
        if not arg_list or self.name not in arg_list:
            if self.get_generic_type_parameter() is bool:
        ...
        if arg_list.count(self.name) > 1:
            return Exception(
                f"Multiple instances of argument '{self.name}' were provided"
            )
